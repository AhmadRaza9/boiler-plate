...
// get event details
$start_date_raw = get_post_meta( get_the_ID(), 'event-start-date', true );
$end_date_raw = get_post_meta( get_the_ID(), 'event-date', true );
...
// get start date and end date from timestamp
$date_start = new DateTime();
$date_start->setTimestamp(intval($start_date_raw));
$date_start->setTime(0, 0, 0, 0);
$start_date = $date_start->getTimestamp();

$date_end = new DateTime();
$date_end->setTimestamp(intval($end_date_raw));
$date_end->setTime(0, 0, 0, 0);
$end_date = $date_end->getTimestamp();

// get start time and end time from timestamp
$time_start = new DateTime();
$time_start->setTimestamp(intval($start_date_raw));
$time_start->setDate(1970, 1, 1);
$start_time = $time_start->getTimestamp();

$time_end = new DateTime();
$time_end->setTimestamp(intval($end_date_raw));
$time_end->setDate(1970, 1, 1);
$end_time = $time_end->getTimestamp();
...
// set custom width for event details section and event info section
...
if ( ($page_image_hide == 'yes') && ($page_info_hide == 'yes') ) {
	$page_meta_section_start = '<div class="vsel-meta vsel-meta-full">';
